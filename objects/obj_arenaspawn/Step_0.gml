    ...
    case states.normal:
        if (obj_player1.x <= x)
        {
            ...
            with (obj_arenagate)
            {
                ...
                image_index = 0
                block_inst = gml_Script_instance_create(x, y, obj_solid)
                with (block_inst)
                {
        ...
        if (!ds_list_empty(baddielist))
        {
            for (var i = 0; i < ds_list_size(baddielist); i++)
            {
                ...
                if (b[0] == wave)
                {
                    ...
                    {
                        with (gml_Script_instance_create(b[1].x, b[1].y, obj_arena_pizzaportal))
                        {
                            stored_id = b[1]
                            alarm[0] = other.round_max
                        }
                        instance_deactivate_object_hook(b[1])
                    }
                }
                else
                    instance_deactivate_object_hook(b[1])
