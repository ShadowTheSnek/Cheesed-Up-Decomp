...
with (obj_player)
{
    ...
    if (other.image_yscale == -1)
    {
        ...
        if ((key_up || forced) && (!(place_meeting(x, (y - 1), obj_destructibles))) && place_meeting(x, (y - 10), other) && (!instance_exists(obj_fadeout)) && (state == states.normal || state == states.pogo || state == states.jump || state == states.mach1 || state == states.mach2 || state == states.mach3 || state == states.Sjumpprep || state == states.ratmount || state == states.ratmountjump || state == states.ratmountbounce || (state == states.punch && sprite_index == spr_breakdanceuppercut) || state == (308 << 0) || forced))
        {
            if (character == "S")
                movespeed = 0
            set_lastroom()
