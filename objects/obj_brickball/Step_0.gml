...
if (speedline == 0)
{
    with (gml_Script_instance_create(x, y, obj_brickballspeedline))
    {
        playerid = other.id
        other.speedline = 1
    }
...
if (hitbox == 0)
{
    with (gml_Script_instance_create(x, y, obj_shotgunbullet))
    {
        brick = 1
        visible = false
...
if (scr_solid((x + image_xscale), y) && kicked && (!(place_meeting((x + image_xscale), y, obj_destructibles))) && (!(check_slope((x + image_xscale), y))) && (((!(place_meeting((x + image_xscale), y, obj_ratblock))) && (!(place_meeting((x + image_xscale), y, obj_metalblock))) && (!(place_meeting((x + image_xscale), y, obj_tntblock)))) || (!buffed)))
    instance_destroy()
