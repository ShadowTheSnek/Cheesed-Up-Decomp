if (state == states.revolver || state == states.boots)
{
    var len = array_length(hub_array)
    var curve = animcurve_channel_evaluate((state == states.revolver ? outback : incubic), anim_t)
    var xx = ((x + 50) - camera_get_view_x(view_camera[0]))
    var yy = ((y - 90) - camera_get_view_y(view_camera[0]))
    var arrowx = xx
    var arrowy = (yy + (40 * curve))
