...
{
    var b = bg_arr[i]
    var bg = layer_background_get_id_fix(layer_get_id(b))
...
switch state
{
    case states.normal:
        ...
        with (towerID)
        {
            ...
            else if (hitY <= room_height)
            {
                ...
                with (gml_Script_instance_create((x + irandom_range(-48, 48)), (259 + irandom_range(-16, 16)), obj_endingeffect))
                {
    ...
    case 2:
        if (!instance_exists(obj_endingcard))
        {
            if (credits_pos < array_length(credits))
            {
                ...
                {
                    with (gml_Script_instance_create(0, 0, obj_endingcard))
                    {
                        if (b[0] == -4)
                        {
                            showsprite = 0
        ...
        else
        {
            ...
            repeat (irandom(3) + 1)
            {
                with (gml_Script_instance_create(((room_width + 300) + irandom(100)), 386, obj_endingprop))
                {
    ...
    case 6:
        ...
        {
            state++
            gml_Script_instance_create(0, 0, obj_endingrank)
