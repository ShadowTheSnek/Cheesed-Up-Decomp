if used
    ...
    image_index = 0
var s = 16
if (place_meeting(x, (y - s), obj_player) && (!(collision_point(((x + sprite_width) + (image_xscale * 32)), (y - 32), obj_solid, 1, 0))))
{
    with (obj_player)
    {
        var by = 4
        if (state == states.mach3 || (state == states.ratmount && ratmount_movespeed >= 12))
            by = 10
        var bx = 8
