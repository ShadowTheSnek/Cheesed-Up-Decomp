...
for (var i = 0; i < array_length(lvl); i++)
{
    ...
    for (var j = 0; j < 5; j++)
        toppins += ini_read_real("Toppin", concat(name, (j + 1)), 0)
    for (j = 0; j < 3; j++)
        achievements += ini_read_real("achievements", concat(name, (j + 1)), 0)
}
for (i = 0; i < array_length(extra_ach); i++)
    achievements += ini_read_real("achievements", extra_ach[i], 0)
...
for (i = 0; i < array_length(_palettes); i++)
    palettes += ini_read_real("Palettes", _palettes[i], 0)
ini_close()
text = 0
depth = 100
grannyID = gml_Script_instance_create(x, y, obj_tutorialbook)
grannyID.showgranny = 0
...
function anon@2122@gml_Object_obj_computer_Create_0() //gml_Script_anon@2122@gml_Object_obj_computer_Create_0
{
    if (text == 0)
    {
        var str = gml_Script_lang_get_value_newline("computer_text1")
        str = embed_value_string(str, [toppins, maxtoppins, secrets, maxsecrets, treasure, maxtreasure])
    }
    else
    {
        str = gml_Script_lang_get_value_newline("computer_text2")
