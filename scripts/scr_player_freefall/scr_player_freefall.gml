function scr_player_freefall() //gml_Script_scr_player_freefall
{
    ...
    if (vsp >= 2)
    {
        ...
        if (vsp > 17)
        {
            if (punch_afterimage > 0 && (global.afterimage == 0 || global.gameplay))
                punch_afterimage--
            else
            {
                punch_afterimage = 5
    ...
    if (grounded && vsp > 0 && (freefallsmash < 10 || (!(place_meeting(x, (y + vsp), obj_metalblock)))) && (!(place_meeting(x, y, obj_destructibles))) && (!(place_meeting(x, (y + vsp), obj_destructibles))) && (!(place_meeting(x, (y + 1), obj_destructibles))) && (!(place_meeting(x, ((y + vsp) + 6), obj_destructibles))) && ((!(place_meeting(x, (y + 1), obj_ladder))) || (!global.gameplay) || (!key_down) || check_solid(x, (y + 1))))
    {
        if scr_slope()
        {
            with (check_slope(x, (y + 1)))
            {
                ...
                particle_set_scale((5 << 0), (-sign(image_xscale)), 1)
        ...
        else
        {
            ...
            if (sprite_index == spr_poundcancel1 || sprite_index == spr_playerSP_poundcancel1)
