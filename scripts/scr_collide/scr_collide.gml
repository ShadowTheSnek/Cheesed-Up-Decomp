var _temp_local_var_2, _temp_local_var_3, _temp_local_var_24, _temp_local_var_34;
function scr_collide() //gml_Script_scr_collide
{
    ...
    while (i < t)
    {
        ...
        {
            var _temp_local_var_34 = abs((target_y - y))
            if (abs((target_y - y)) <= 0)
            {
            }
            else
            {
                while (true)
                {
                    ...
                    {
                        y += sv
                        var _temp_local_var_34 = (abs((target_y - y)) - 1)
                        if (abs((target_y - y)) - 1)
                            continue
                        break
                    }
    ...
    while (i < t)
    {
        ...
        {
            var _temp_local_var_24 = abs((target_x - x))
            if (abs((target_x - x)) <= 0)
            {
            }
            else
            {
                while (true)
                {
                    ...
                    {
                        x += sh
                        var _temp_local_var_24 = (abs((target_x - x)) - 1)
                        if (abs((target_x - x)) - 1)
                            continue
                        break
                    }
    ...
    if (platformid != -4 || (place_meeting(x, (y + flip), obj_movingplatform) && (!(place_meeting(x, (y - (2 * flip)), obj_movingplatform)))))
        grounded = 1
