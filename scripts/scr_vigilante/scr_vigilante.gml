...
function scr_vigilante_update_sounds() //gml_Script_scr_vigilante_update_sounds
{
    if (sprite_index == spr_vigilante_flamethrower)
    {
        ...
        fmod_event_instance_stop(snd_flame, 0)
    if (state == states.mach2 && sprite_index != spr_playerV_crouchmove)
    {
        ...
        fmod_event_instance_stop(snd_mach1, 0)
    ...
    if (sprite_index == spr_vigilante_uziprepare)
    {
        ...
        fmod_event_instance_stop(snd_uziprep, 0)
    ...
    {
        if (music != noone)
            fmod_event_instance_set_parameter(music.event, "state", other.songstate, 1)
    }
}
...
function scr_vigilante_arenaintro() //gml_Script_scr_vigilante_arenaintro
{
    if (skipintro && (!introwait))
    {
        ...
        if (!check_char("V"))
        {
            with (gml_Script_instance_create(x, y, obj_pistolpickup))
            {
    ...
    else
    {
        ...
        else if (sprite_index != spr_vigilante_impatient)
        {
            ...
            image_index = (image_number - 1)
        if (global.pistol || (check_char("V") && obj_player1.state == states.normal))
        {
            sprite_index = spr_playerV_idle
            state = states.walk
        }
...
function scr_vigilante_phase1hurt() //gml_Script_scr_vigilante_phase1hurt
{
    ...
    else
    {
        ...
        gml_Script_instance_create(0, 0, obj_pizzahead_whitefade)
        obj_camera.lock = 0
        camzoom = 1
...
function scr_vigilante_walk() //gml_Script_scr_vigilante_walk
{
    ...
    if (cooldown <= 0 && flickertime <= 0 && (!reposition))
    {
        ...
        {
            case (0 << 0):
                scr_vigilante_do_revolver(1, attack[1], 0)
                break
            case (10 << 0):
            ...
            case (3 << 0):
                ...
                sprite_index = spr_vigilante_intro2
                image_index = 16
                with (gml_Script_instance_create(464, 195, obj_ladderhorizontal))
                {
...
function scr_vigilante_throw_cow() //gml_Script_scr_vigilante_throw_cow
{
    if (shot > 0)
    {
        ...
        with (gml_Script_instance_create((x + (image_xscale * 100)), (y - 10), obj_vigilantecow))
        {
            xscale = other.image_xscale
            vsp = -16
            spd = 6
...
function scr_vigilante_throw_dynamite() //gml_Script_scr_vigilante_throw_dynamite
{
    if (shot > 0)
    {
        ...
        if (obj_player.x != x)
            image_xscale = (-(sign((x - obj_player.x))))
        with (gml_Script_instance_create((x + (image_xscale * 20)), (y - 16), obj_vigilantedynamite))
        {
...
function scr_vigilante_revolver() //gml_Script_scr_vigilante_revolver
{
    ...
    if (sprite_index == spr_playerV_revolverhold)
    {
        ...
        else
        {
            ...
            image_index = 0
            hsp = (image_xscale * 4)
            with (gml_Script_instance_create((x + (image_xscale * 80)), (y + 10), obj_vigilanteshot))
            {
        ...
        switch duelphase
        {
            ...
            case 2:
                ...
                else
                {
                    ...
                    with (gml_Script_instance_create((x - (len * 0.8)), y, obj_crazyrunothereffect))
                        image_speed = 0.25
                    with (gml_Script_instance_create((x - (len * 0.2)), y, obj_crazyrunothereffect))
                        image_speed = 0.25
                    var xx = 0
                    var l = (5 + irandom(2))
                    repeat l
                    {
                        with (create_debris(((x - len) + xx), y, spr_genericpoofeffect, 1))
                        {
                            destroyonanimation = 1
                            hsp = 0
    ...
    if (sprite_index != spr_vigilante_flamethrowerturn)
    {
        if (sprite_index == spr_vigilante_flamethrowerintro)
        {
            ...
            if (floor(image_index) == (image_number - 1))
            {
                ...
                image_index = 0
                flameID = gml_Script_instance_create(x, y, obj_flamethrowerhurtbox)
                with (flameID)
                {
                    sprite_index = spr_vigilante_flamethrower
                    mask_index = spr_vigilante_flamethrower
                    image_xscale = other.image_xscale
                    with (gml_Script_instance_create(x, y, obj_lightsource_attach))
                    {
                        objectID = other.id
                        distance = 500
                    }
    ...
    else
    {
        ...
        if (sprite_index == spr_vigilante_uzi)
        {
            ...
            else
            {
                ...
                if (dir > 3)
                    dir = 0
                with (gml_Script_instance_create(x, y, obj_uziprojectile))
                {
...
function scr_vigilante_bazooka() //gml_Script_scr_vigilante_bazooka
{
    ...
    if (sprite_index == spr_vigilante_bazooka)
    {
        ...
        if (floor(image_index) >= 11 && (!shot))
        {
            shot = 1
            vsp = -6
            with (gml_Script_instance_create((x + (image_xscale * 10)), y, obj_vigilantebazooka))
                image_xscale = other.image_xscale
...
function scr_vigilante_crate() //gml_Script_scr_vigilante_crate
{
    ...
    {
        shot = 1
        with (gml_Script_instance_create(obj_player1.x, -64, obj_vigilantecrate))
            cow = 1
    }
    if (floor(image_index) == (image_number - 1))
        state = states.walk
