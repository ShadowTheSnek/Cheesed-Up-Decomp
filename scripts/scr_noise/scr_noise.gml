...
function scr_noise_walk() //gml_Script_scr_noise_walk
{
    ...
    if (!droptrap)
    {
        ...
        else if (flickertime <= 0)
        {
            ...
            gml_Script_instance_create(0, 0, obj_noiseballooncrash)
    ...
    if (!golf)
    {
        ...
        else if (machbuffer == 0 && grounded && sprite_index == spr_playerN_mach)
        {
            ...
            if (phase == 2)
            {
                ...
                image_index = 4
                with (gml_Script_instance_create((x + (image_xscale * 20)), y, obj_skateboardnoise))
...
function scr_noise_pogo() //gml_Script_scr_noise_pogo
{
    ...
    {
        with (gml_Script_instance_create(x, y, obj_noisebombboss))
        {
            vsp = -11
...
function scr_noise_drop() //gml_Script_scr_noise_drop
{
    ...
    else
    {
        ...
        image_index = 0
        with (gml_Script_instance_create(x, y, obj_noisehotairempty))
            image_xscale = other.image_xscale
    }
    if (dropcooldown > 0)
        dropcooldown--
    ...
    switch substate
    {
        ...
        case (106 << 0):
            ...
            if (hsp == 0 && (!instance_exists(obj_noisebosscrate)))
                gml_Script_instance_create(x, -100, obj_noisebosscrate)
        ...
        case (66 << 0):
            if (floor(image_index) == (image_number - 1))
            {
                ...
                gml_Script_instance_create((room_width + 100), y, obj_noisettefinale)
            }
            break
        case (69 << 0):
            with (obj_player1)
...
function scr_noise_phase1hurt() //gml_Script_scr_noise_phase1hurt
{
    sprite_index = spr_playerN_hurt
    image_index = 0.35
    scr_boss_phase1hurt(function() //gml_Script_anon@28609@scr_noise_phase1hurt_scr_noise
    {
