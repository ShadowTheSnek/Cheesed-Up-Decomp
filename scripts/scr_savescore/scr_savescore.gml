function scr_savescore(argument0) //gml_Script_scr_savescore
{
    ...
    else if ((global.collect + global.collectN) >= global.srank)
    {
        ...
        global.rank = "d"
    scr_play_rank_music()
    if (global.saveloaded && ((!global.MOD.OldLevels) || argument0 == "snickchallenge") && (!global.MOD.DoubleTrouble))
    {
        ...
        if (ini_read_real("Highscore", argument0, 0) < global.collect)
        {
            ...
            ini_write_real("Laps", argument0, global.laps)
        }
...
function scr_play_rank_music() //gml_Script_scr_play_rank_music
{
    ...
    if ((room != tower_entrancehall || global.exitrank) && room != tower_tutorial1 && room != sugarytut_1)
    {
        ...
        fmod_event_instance_set_parameter(global.snd_rank, "rank", s, 1)
    }
}
