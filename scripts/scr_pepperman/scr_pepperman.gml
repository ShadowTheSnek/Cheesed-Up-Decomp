...
function scr_pepperman_sounds() //gml_Script_scr_pepperman_sounds
{
    if (state == states.shoulderbash)
    {
        ...
        fmod_event_instance_stop(snd_shoulderbash, 1)
    if (state == states.freefallprep)
    {
        ...
        fmod_event_instance_stop(snd_groundpound, 1)
    ...
    {
        if (music != noone)
            fmod_event_instance_set_parameter(music.event, "state", other.shrinkstate, 1)
    }
}
...
function scr_pepperman_spawn_knight() //gml_Script_scr_pepperman_spawn_knight
{
    if (knightdir == -1)
    {
        with (gml_Script_instance_create((room_width + 10), 416, obj_peppermanGIANTbowlingball))
            image_xscale = -1
    }
    if (knightdir == 1)
    {
        with (gml_Script_instance_create(-10, 416, obj_peppermanGIANTbowlingball))
            image_xscale = 1
    }
    knightdir *= -1
}
...
function scr_pepperman_shuffle_adds() //gml_Script_scr_pepperman_shuffle_adds
{
    if (bowlingballcount <= 0)
    {
        ...
        else if ((!throwknight) && (!instance_exists(obj_peppermanGIANTbowlingball)))
        {
            bowlingballcount = MAX_bowlingballcount
            throwknight = (!throwknight)
            with (gml_Script_instance_create(x, y, obj_peppermanbowlingballspawner))
                balls = 1
        }
    }
}
...
function scr_pepperman_rolling() //gml_Script_scr_pepperman_rolling
{
    if (sprite_index != spr_pepperman_groundpoundland)
    {
        ...
        {
            gml_Script_instance_create((x + (image_xscale * 20)), y, obj_bangeffect)
            image_xscale *= -1
            vsp = -10
...
function scr_pepperman_shoulderbash() //gml_Script_scr_pepperman_shoulderbash
{
    ...
    if (attackspeed >= a_treshold)
    {
        ...
        if (woosh == 0 && phase == 2)
        {
            particle_set_scale((2 << 0), image_xscale, 1)
...
function scr_pepperman_phase1hurt() //gml_Script_scr_pepperman_phase1hurt
{
    ...
    image_speed = 0.35
    scr_boss_phase1hurt(function() //gml_Script_anon@16533@scr_pepperman_phase1hurt_scr_pepperman
    {
    }
)
}
