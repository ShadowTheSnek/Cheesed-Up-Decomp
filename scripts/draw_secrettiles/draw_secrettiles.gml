function draw_secrettiles(argument0) //gml_Script_draw_secrettiles
{
    with (argument0)
    {
        shader_reset()
        var mario = (global.secrettiles && (!instance_exists(obj_fakeplayer)))
        if ((!(bbox_in_camera(view_camera[0], 32))) || (image_alpha <= 0 && (!mario)) || array_length(tiles) == 0)
            return;
        if ((((!mario) && image_alpha < 1) || global.secrettile_clip_distance > 0) && (!global.performance))
        {
            shader_set_fix(shd_secrettile)
            ...
            shader_set_uniform_f(alphafix, 0)
            if mario
            {
        ...
        function anon@1712@draw_secrettiles_draw_secrettiles(argument0) //gml_Script_anon@1712@draw_secrettiles_draw_secrettiles
        {
            draw_tile(argument0.tileset, argument0.tile_data, 0, argument0.x, argument0.y)
        }
