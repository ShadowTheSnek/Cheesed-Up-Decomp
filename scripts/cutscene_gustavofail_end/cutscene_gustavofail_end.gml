function cutscene_gustavofail_end(argument0) //gml_Script_cutscene_gustavofail_end
{
    ...
    with (obj_player1)
    {
        ...
        {
            gml_Script_instance_create((x - 20), (y - 20), obj_slapstar)
            with (gml_Script_instance_create((x - 20), (y - 20), obj_smallnumber))
            {
                negative = 1
                number = "-50"
            }
            ...
            if (global.collect != 0)
            {
                if (character == "P" || character == "V")
                {
                    repeat (10)
                    {
                        with (gml_Script_instance_create(x, y, obj_pizzaloss))
