function scr_player_tumble() //gml_Script_scr_player_tumble
{
    ...
    if (sprite_index == spr_tumblestart)
        movespeed = 6
    if ((!grounded) && (sprite_index == crouchslipspr || sprite_index == machrollspr || sprite_index == spr_mach2jump || sprite_index == spr_backslide || sprite_index == spr_backslideland) && (!((character == "N" && noisetype == 1))))
    {
        vsp = 10
    ...
    if (character == "P" && floor(image_index) == (image_number - 1) && sprite_index == machrollspr && movespeed > 12)
    {
    ...
    if ((state != states.freefall && (check_solid((x + xscale), y) || scr_solid_slope((x + xscale), y)) && (!(place_meeting((x + hsp), y, obj_rollblock))) && (!(place_meeting((x + hsp), y, obj_unbumpablewall))) && ((!(place_meeting((x + hsp), y, obj_rattumble))) || sprite_index != spr_tumble) && (!(place_meeting((x + hsp), y, obj_destructibles))) && ((!(place_meeting((x + hsp), y, obj_metalblock))) || character != "SP" || sprite_index != spr_machroll || abs(hsp) < 12)) || place_meeting(x, y, obj_timedgate))
    {
        hsp = 0
        movespeed = 0
    ...
    if (sprite_index == crouchslipspr || sprite_index == spr_breakdancesuper || sprite_index == machrollspr || sprite_index == spr_tumble || sprite_index == spr_tumblestart || sprite_index == spr_player_machroll || sprite_index == spr_player_mach2jump)
