function scr_player_ratmount() //gml_Script_scr_player_ratmount
{
    ...
    if (input_buffer_jump > 0 && can_jump && (gusdashpadbuffer == 0 || check_char("G")) && state != states.ratmountskid)
    {
        input_buffer_jump = 0
    ...
    {
        var p = gml_Script_instance_create(x, y, obj_dashcloud)
        with (p)
        {
    ...
    if ((key_down && grounded && vsp > 0 && gusdashpadbuffer <= 0) || scr_solid(x, y))
    {
        if (check_char("G") && ratmount_movespeed >= 12)
        {
            ...
            crouchslipbuffer = 25
            sprite_index = spr_lonegustavo_roll
            machhitAnim = 0
            state = states.tumble
...
function ratmount_dotaunt() //gml_Script_ratmount_dotaunt
{
    if (key_taunt2 && state != states.backbreaker && brick && gusdashpadbuffer == 0)
    {
        ...
        if (((!key_up) || (!supercharged)) && global.tauntcount < 10 && place_meeting(x, y, obj_exitgate) && (global.panic == 1 || instance_exists(obj_wartimer)) && global.combotime > 0 && global.leveltosave != "grinch")
        {
            if global.gameplay
...
function ratmount_kickbrick() //gml_Script_ratmount_kickbrick
{
    ...
    with (gml_Script_instance_create((x + (image_xscale * _pad)), y, obj_brickball))
    {
    ...
    if (key_shoot2 && ratpowerup != -4 && ratshootbuffer <= 0)
    {
        switch ratpowerup
        {
            case obj_noisegoblin:
                with (gml_Script_instance_create((x + (20 * xscale)), y, obj_playernoisearrow))
                    direction = point_direction(x, y, (x + (other.xscale * 4)), y)
                break
            case obj_smokingpizzaslice:
                with (gml_Script_instance_create((x + (20 * xscale)), (y + 20), obj_playersmokehitbox))
                {
                ...
                break
            case obj_spitcheese:
                with (gml_Script_instance_create((x + (20 * xscale)), y, obj_playerspikecheese))
                {
                    spd += other.movespeed
                    image_xscale = other.xscale
                }
