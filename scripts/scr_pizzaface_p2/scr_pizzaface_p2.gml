...
function scr_pizzaface_p2_update_sounds() //gml_Script_scr_pizzaface_p2_update_sounds
{
    if instance_exists(obj_pizzahead_haywire)
    {
        ...
        fmod_event_instance_stop(snd_haywire, 1)
    if (state == (237 << 0))
    {
        ...
        if (floor(image_index) < 24)
            ...
            s = 3
        fmod_event_instance_set_parameter(snd_tvthrow, "state", s, 1)
    }
    else if (fmod_event_instance_get_parameter(snd_tvthrow, "state") < 3)
        fmod_event_instance_stop(snd_tvthrow, 1)
    ...
    if (sprite_index == spr_pizzahead_pullinglevel1 && floor(image_index) <= 21)
    {
        ...
        fmod_event_instance_stop(snd_grablevel, 1)
    if (sprite_index == spr_pizzahead_grabitem && floor(image_index) >= 8)
    {
        ...
        fmod_event_instance_stop(snd_fishing, 0)
    if instance_exists(obj_pizzaheadbrickball)
    {
        ...
        fmod_event_instance_stop(snd_ratball, 1)
}
...
function scr_pizzaface_p2_fall() //gml_Script_scr_pizzaface_p2_fall
{
    ...
    switch substate
    {
        ...
        case (135 << 0):
            ...
            {
                laugh_snd = 0
                gml_Script_instance_create(0, 0, obj_pizzafaceboss_p2background)
                var lay = layer_get_id("Backgrounds_2")
                var bg = layer_background_get_id_fix(lay)
                alarm[7] = 5
                layer_set_visible("Backgrounds_1", 1)
                layer_set_visible("Backgrounds_zigzag1", 1)
        ...
        else
        {
            ...
            switch t
            {
                case (0 << 0):
                    ...
                    image_index = 0
                    shot = 0
                    gml_Script_instance_create((x + (42 * image_xscale)), (y + 200), obj_pizzaheadswordstone)
                    with (obj_camera)
                    {
                        shake_mag = 3
...
function boss_pizzahead_portraitthrow() //gml_Script_boss_pizzahead_portraitthrow
{
    ...
    {
        shot = 1
        with (gml_Script_instance_create(x, (y - 32), obj_pizzaheadportrait))
        {
...
function boss_pizzahead_pullinglevel() //gml_Script_boss_pizzahead_pullinglevel
{
    ...
    if (floor(image_index) >= 22 && sprite_index == spr_pizzahead_pullinglevel1 && (!shot))
    {
        ...
        repeat (7 + irandom(5))
        {
            ...
            with (gml_Script_instance_create(_x, (room_height + 100), obj_forknight_pizzahead))
...
function boss_pizzahead_throwing() //gml_Script_boss_pizzahead_throwing
{
    ...
    else
    {
        ...
        else if (sprite_index == spr_pizzahead_ratballthrow)
        {
            ...
            {
                shot = 1
                with (gml_Script_instance_create((x + (46 * image_xscale)), (y - 64), obj_pizzaheadbrickball))
                {
        ...
        case states.mini:
            ...
            else
            {
                ...
                if (ammo > 0)
                {
                    ...
                    with (gml_Script_instance_create(x, y, obj_pizzaheadbullet))
                    {
                ...
                else
                {
                    ...
                    with (gml_Script_instance_create((x - (image_xscale * 30)), y, obj_pizzaheadminigun))
                    {
                        image_xscale = other.image_xscale
        ...
        case (74 << 0):
            if (floor(image_index) == (image_number - 1))
            {
                if (sprite_index == spr_pizzahead_grabbox)
                {
                    ...
                    image_index = 0
                    idle_buffer = 0
                    with (gml_Script_instance_create(x, (y - 15), obj_pistolpickup))
                    {
                ...
                else if (sprite_index == spr_pizzahead_grabdynamite)
                {
                    ...
                    repeat (5)
                    {
                        with (gml_Script_instance_create((x - (image_xscale * 5)), (y - 15), obj_vigilantedynamite))
                        {
                            hsp = (irandom_range(5, 10) * (-other.image_xscale))
                            vsp = (-(irandom_range(7, 14)))
                        }
...
function boss_pizzahead_p2_phase1hurt() //gml_Script_boss_pizzahead_p2_phase1hurt
{
    ...
    if (buildup > 0)
    {
        ...
        _x = clamp((x - ((obj_screensizer.actual_width * camzoom) / 2)), 0, (room_width - (obj_screensizer.actual_width * camzoom)))
