function noise_start_round() //gml_Script_noise_start_round
{
    ...
    {
        with (gml_Script_instance_create(256, 416, obj_baddiespawner))
        {
            create_particle(x, y, (9 << 0), 0)
            content = obj_noisey
        }
        with (gml_Script_instance_create(672, 416, obj_baddiespawner))
        {
            create_particle(x, y, (9 << 0), 0)
            content = obj_noisey
            image_xscale = -1
...
function mrstick_start_round() //gml_Script_mrstick_start_round
{
    with (obj_mrstickboss)
    {
        ...
        if (b != -4)
        {
            ...
            var xx = (tx * 32)
            with (gml_Script_instance_create((x + xx), y, b[0]))
            {
