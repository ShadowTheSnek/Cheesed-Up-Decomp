...
function tdp_input_update(argument0) //gml_Script_tdp_input_update
{
    ...
    for (var i = 0; i < num; i++)
    {
        ds_map_find_value(global.input_list, key).ds_map_find_value(global.input_list, key).update(argument0)
...
function tdp_action(argument0, argument1, argument2) //gml_Script_tdp_action
{
    ...
    return 
    {
        var type: argument0,
        var value: argument1,
        var joystick_direction: argument2
    };
}

function tdp_input_serialize_key() //gml_Script_tdp_input_serialize_key
{
...
function tdp_input_deserialize(argument0, argument1) //gml_Script_tdp_input_deserialize
{
    ...
    in = new tdp_input_key(argument0)
    i = 0
    while (i < array_length(arr))
    {
        ...
        else
        {
            ...
            if (type == (2 << 0))
            {
                joystick_direction = real(arr[(i + 2)])
                i++
...
{
    arr = array_create(0)
    for (i = 0; i < array_length(argument0.actions); i++)
    {
...
function tdp_get_tutorial_icon(argument0) //gml_Script_tdp_get_tutorial_icon
{
    ...
    for (i = 0; i < array_length(in.actions); i++)
    {
        q = tdp_get_icon(in.actions[i])
        if (q != -4)
            return q;
    }
    return -4;
