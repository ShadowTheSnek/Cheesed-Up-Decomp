    ...
    else if (combo_count > 0)
    {
        ...
        switch state
        {
            ...
            case (166 << 0):
                ...
                with (gml_Script_instance_create(x, y, obj_millionpunchhitbox))
                    baddieID = other.id
                break
            case (2 << 0):
                dynamite_shot = 1
...
function boss_vigilante_revolver() //gml_Script_boss_vigilante_revolver
{
    ...
    if (image_index > (image_number - 1))
    {
        if (sprite_index == spr_playerV_revolverstart || sprite_index == spr_playerV_airrevolverstart)
        {
            ...
            image_index = 0
            with (gml_Script_instance_create((x + (image_xscale * 20)), (y + 20), obj_revolvershot))
                image_xscale = other.image_xscale
            revolver_count--
            revolver_jumpcount++
...
function boss_vigilante_superattack() //gml_Script_boss_vigilante_superattack
{
    ...
    {
        duel_buffer = -1
        duelinst = gml_Script_instance_create((room_width / 2), (room_height / 2), obj_cloudeffect)
        with (duelinst)
        {
    ...
    with (lastplayerid)
    {
        ...
        {
            other.duel_buffer = -2
            gml_Script_instance_create((x + (image_xscale * 20)), y, obj_shotgunbullet)
