...
function get_percentage() //gml_Script_get_percentage
{
    ...
    for (var i = 0; i < array_length(levels); i++)
    {
        ...
        if (ini_read_real("Highscore", level, 0) > 0)
            count++
        if ini_read_real("Treasure", level, 0)
            count++
        for (var j = 0; j < 5; j++)
        {
            if ini_read_real("Toppin", concat(level, (j + 1)), 0)
                count++
        }
        ...
        for (j = 0; j < 3; j++)
        {
            if ini_read_real("achievements", concat(level, (j + 1)), 0)
                ac++
        }
        if (ac >= 3)
            extracount++
    ...
    {
        var boss = bosses[i]
        if ini_read_real(boss, "bosskey", 0)
            count++
    }
    ...
    {
        if ini_read_real("achievements", bossach[i], 0)
            extracount++
    }
