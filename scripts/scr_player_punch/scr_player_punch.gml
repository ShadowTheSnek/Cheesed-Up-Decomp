function scr_player_punch() //gml_Script_scr_player_punch
{
    if (sprite_index == spr_breakdanceuppercut || sprite_index == spr_breakdanceuppercutend)
    {
        ...
        if (punch_afterimage > 0)
            punch_afterimage--
        else if (vsp < 0 || global.afterimage == 1)
        {
            punch_afterimage = 5
    ...
    else if (string_pos("breakdance", sprite_get_name(sprite_index)) || string_pos("buttattack", sprite_get_name(sprite_index)))
    {
        ...
        if (breakdance > 0)
            breakdance--
        landAnim = 0
        if (movespeed <= 0 && (sprite_index == spr_breakdancesuper || sprite_index == spr_buttattackend))
            state = states.normal
        ...
        if (sprite_index == spr_breakdancesuper)
            ...
            image_speed = 0.4
        if (scr_solid((x + xscale), y) && (!(check_slope((x + sign(hsp)), y))) && (!(place_meeting((x + xscale), y, obj_destructibles))) && ((!(place_meeting((x + xscale), y, obj_destructibles))) || (sprite_index != spr_buttattack && sprite_index != spr_buttattackstart)))
        {
            if ledge_bump(32)
            {
                if (sprite_index == spr_buttattack || sprite_index == spr_buttattackstart || sprite_index == spr_buttattackend)
                {
    ...
    else
    {
        ...
        if (breakdance > 0)
            breakdance--
        var bump = (sprite_index == spr_kungfujump || sprite_index == spr_lungehit)
        if (move != 0)
        {
        ...
        var _kungfuair = (sprite_index == spr_kungfuair1 || sprite_index == spr_kungfuair2 || sprite_index == spr_kungfuair3 || sprite_index == spr_kungfuair1transition || sprite_index == spr_kungfuair2transition || sprite_index == spr_kungfuair3transition)
        ...
        else if (key_attack && move != 0 && global.gameplay)
            movespeed += 0.02
        if ((_kungfuground || _kungfuair) && input_buffer_slap > 0)
        {
            input_buffer_slap = 0
            scr_perform_move((1 << 0), (80 << 0))
            return;
        }
        ...
        if (key_down && (_kungfuair || _kungfuground))
        {
            particle_set_scale((5 << 0), xscale, 1)
            create_particle(x, y, (5 << 0), 0)
            state = states.tumble
        ...
        {
            if (grounded && vsp >= 0 && (image_index >= (image_number - 2) || sprite_index != spr_lungehit))
            {
        ...
        if ((!bump) && (check_solid((x + xscale), (y - (grounded * 16))) || scr_solid_slope((x + xscale), (y - (grounded * 16)))) && (!(check_slope(x, (y - 1)))) && (!(place_meeting((x + xscale), y, obj_destructibles))))
        {
            if ledge_bump(32)
            {
                if (((!grounded) || scr_slope()) && (!((character == "N" && noisetype == 1))))
                {
