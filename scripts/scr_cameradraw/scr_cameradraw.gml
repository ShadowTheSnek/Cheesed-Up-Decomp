function scr_cameradraw() //gml_Script_scr_cameradraw
{
    ...
    if (player.state != states.gameover)
    {
        var hide = ((((global.MOD.Mirror && player.x > (obj_screensizer.actual_width - 250)) || ((!global.MOD.Mirror) && player.x < 250)) && player.y < 169) || manualhide)
        if sugary
        ...
        if bo
            cmb = 1
        if (global.gameplay || global.heatmeter)
        ...
        if global.heatmeter
        {
            ...
            pal_swap_set(heatpal, (min(global.stylethreshold, 3) + (global.stylethreshold >= 3 && global.style >= 55)), 0)
        ...
        if (global.shootstyle == 1 && showbullet)
        {
            ...
            for (i = 0; i < max(global.bullet, 3); i++)
            {
                var a = alpha
                var img = bulletimage
                var col = c_white
        ...
        if bo
        {
            draw_sprite_ext(spr_inv, -1, 41, (150 + hud_posY), 1, 1, 1, c_white, alpha)
            if global.key_inv
                draw_sprite_ext(spr_key, -1, 41, (150 + hud_posY), 1, 1, 1, c_white, alpha)
        }
    }
}
