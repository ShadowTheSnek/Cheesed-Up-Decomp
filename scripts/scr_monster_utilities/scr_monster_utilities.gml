...
function scr_monster_stop_music() //gml_Script_scr_monster_stop_music
{
    with (obj_music)
    {
        if fmod_event_instance_is_playing(kidspartychaseID)
        {
            ...
            if (current_custom != -4 && instance_exists(obj_cyop_loader))
            {
...
function scr_monster_activate() //gml_Script_scr_monster_activate
{
    ...
    with (obj_monstergate)
    {
        ...
        {
            active = 1
            instance_deactivate_object_hook(solidID)
            image_speed = 0.35
        }
    }
    ...
    with (obj_music)
    {
        if ((!global.panic) && (!instance_exists(obj_ghostcollectibles)) && (!fmod_event_instance_is_playing(kidspartychaseID)))
        {
            ...
            if (current_custom != -4 && instance_exists(obj_cyop_loader))
            {
                song = custom_music[current_custom]
                if song.fmod
                {
                    savedmusicpause = fmod_event_instance_get_paused(song.instance)
                    fmod_event_instance_set_paused(song.instance, 1)
                }
                else
                {
    ...
    switch investigatestate
    {
        ...
        case 2:
            ...
            else
            {
                state = (219 << 0)
                image_xscale *= -1
                gml_Script_instance_create(x, y, obj_patroller)
            }
            break
    }
