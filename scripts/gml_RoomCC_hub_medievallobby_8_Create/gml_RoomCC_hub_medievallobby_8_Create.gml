...
pb_dialog[1] = [dialog_create("Welcome to the bomb store."), dialog_create("Would you like to purchase the only item in stock?"), dialog_create("There you go buddy", -4, function() //gml_Script_anon@657@gml_RoomCC_hub_medievallobby_8_Create
{
    global.ruincutscene2 = 1
    quick_ini_write_real(get_savefile_ini(), "cutscene", "ruin2", 1)
    if (!instance_exists(obj_baddiespawner))
    {
        with (gml_Script_instance_create(864, 960, obj_baddiespawner))
            content = obj_pizzagoblinbomb
    }
}
)]
function anon@935@gml_RoomCC_hub_medievallobby_8_Create() //gml_Script_anon@935@gml_RoomCC_hub_medievallobby_8_Create
{
    var no_money = 0
    if no_money
