function scr_player_grab() //gml_Script_scr_player_grab
{
    ...
    if (sprite_index == spr_haulingland && floor(image_index) == (image_number - 1))
        sprite_index = spr_haulingidle
    if ((input_buffer_slap > 0 || input_buffer_grab > 0) && sprite_index != spr_swingding)
    {
        input_buffer_slap = 0
        input_buffer_grab = 0
        if (move != 0)
    ...
    else if (((input_buffer_slap > 0 || input_buffer_grab > 0) && sprite_index == spr_swingding && swingdingendcooldown > 20) || swingdingthrow)
    {
