...
function scr_pizzaface_update_sounds() //gml_Script_scr_pizzaface_update_sounds
{
    if (state == states.ram && (substate == states.arenaintro || substate == states.ram))
    {
        ...
        fmod_event_instance_stop(snd_ram, 1)
    ...
    if (state == states.arenaintro)
    {
        ...
        fmod_event_instance_stop(snd_wind, 1)
}
    ...
    if (flickertime <= 0)
    {
        ...
        else if (floor(image_index) == (image_number - 1))
        {
            if (sprite_index == spr_pizzaface_nosespit1)
            {
                ...
                with (gml_Script_instance_create(x, y, b[0]))
                {
                    important = 1
    ...
    switch substate
    {
        ...
        case (230 << 0):
            ...
            if ((vsp > 0 && grounded) || check_solid((x + sign(hsp)), y))
            {
                ...
                {
                    b = baddie_arr[irandom_range(0, (baddie_range - 1))]
                    with (gml_Script_instance_create(irandom_range(50, (room_width - 50)), (room_height + 30), b[0]))
                    {
                        important = 1
                        sprite_index = b[1]
                        state = states.pizzaheadjump
    ...
    switch substate
    {
        ...
        case (137 << 0):
            if (floor(image_index) == (image_number - 1))
            {
                ...
                with (gml_Script_instance_create(x, y, obj_pizzafaceboss_p2))
                {
                    vsp = 0
                    hsp = 0
                    state = states.fall
