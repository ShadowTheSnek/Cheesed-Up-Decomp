function scr_player_skateboard() //gml_Script_scr_player_skateboard
{
    ...
    if key_jump
    {
        state = states.mach2
        vsp = -6
        with (gml_Script_instance_create((x + (xscale * movespeed)), y, obj_skateboard))
        {
            xscale = other.xscale
            hsp = other.hsp
            vsp = -5
    ...
    if (scr_solid((x + 1), y) && xscale == 1)
    {
        ...
        gml_Script_instance_create(x, (y + 10), obj_skateboarddebris1)
        gml_Script_instance_create(x, (y + 10), obj_skateboarddebris2)
    }
