pal_swap_init_system(shd_pal_swapper)
global.roommessage = "PIZZA TOWER ISLAND"
with (obj_secretbigblock)
    particlespr = spr_streetdebris
if global.panic
{
    var lay = layer_get_id("Backgrounds_stillH1")
    layer_background_sprite(layer_background_get_id_fix(lay), bg_cityfire2)
    var lay2 = layer_get_id("Backgrounds_stillH2")
    layer_background_sprite(layer_background_get_id_fix(lay2), bg_cityfire1)
    var lay3 = layer_get_id("Backgrounds_stillscroll")
    layer_background_sprite(layer_background_get_id_fix(lay3), bg_cityfire3)
    var lay4 = layer_get_id("Backgrounds_scroll")
    layer_background_sprite(layer_background_get_id_fix(lay4), bg_cityfiresmoke)
}
