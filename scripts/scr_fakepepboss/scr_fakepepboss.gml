var e, _temp_local_var_1, _temp_local_var_15, _temp_local_var_154;
...
function fakepep_get_attack(argument0, argument1) //gml_Script_fakepep_get_attack
{
    ...
    var _temp_local_var_154 = @@try_unhook@@()
}
...
function scr_fakepep_update_sounds() //gml_Script_scr_fakepep_update_sounds
{
    ...
    if (state == states.freefall || state == states.jump)
    {
        ...
        fmod_event_instance_stop(snd_bodyslam, 0)
    if (state == states.mach2)
    {
        ...
        fmod_event_instance_stop(snd_mach, 0)
    if ((state == states.Sjumpprep || state == states.Sjump) && sprite_index != spr_fakepeppino_idle)
    {
        ...
        if (state == states.Sjumpprep)
            ...
            s = 1
        fmod_event_instance_set_parameter(snd_superjump, "state", s, 1)
        ...
        fmod_event_instance_stop(snd_superjump, 0)
    if (sprite_index == spr_fakepeppino_throwhead && floor(image_index) >= 6)
    {
        ...
        if (floor(image_index) >= 13)
            s = 1
        fmod_event_instance_set_parameter(snd_headoff, "state", s, 1)
        sound_instance_move(snd_headoff, x, y)
    }
    else
        fmod_event_instance_stop(snd_headoff, 0)
    if (sprite_index == spr_fakepeppino_flailing)
    {
        ...
        fmod_event_instance_stop(snd_flailing, 0)
}
...
function scr_fakepepboss_do_projectiles(argument0, argument1) //gml_Script_scr_fakepepboss_do_projectiles
{
    ...
    switch _attack.attack
    {
        case (0 << 0):
        case (1 << 0):
            with (gml_Script_instance_create(x, y, obj_fakepepclone))
            {
                attack = _attack
        ...
        case (2 << 0):
            ...
            if (_attack.direction == 1)
                ...
                xx = 862
            with (gml_Script_instance_create(xx, yy, obj_fakepepclone))
            {
                attack = _attack
        ...
        case (3 << 0):
            ...
            {
                with (gml_Script_instance_create(0, 0, obj_fakepepspawner))
                {
                    attack = _attack
        ...
        case (4 << 0):
            ...
            if (_attack.direction == -1)
                xx = _attack.x2
            with (gml_Script_instance_create(xx, _attack.y, obj_fakepepclone))
            {
                attack = _attack
            ...
            break
        case (5 << 0):
            with (gml_Script_instance_create(_attack.x1, _attack.y, obj_fakepepclone))
            {
                attack = _attack
                image_xscale = 1
            }
            with (gml_Script_instance_create(_attack.x2, _attack.y, obj_fakepepclone))
            {
                attack = _attack
                image_xscale = -1
            }
...
function scr_fakepepboss_walk() //gml_Script_scr_fakepepboss_walk
{
    ...
    if formed
    {
        ...
        {
            attack = fakepep_get_attack((phase - 1), wastedhits)
    ...
    else if check_solid(x, (y + (sign(vsp) * yy)))
    {
        ...
        if (superjumpbounce > 0 || superjumpdir == 0)
        {
            ...
            if (hitvsp > 0)
            {
                ...
                with (gml_Script_instance_create(x, (y + 30), obj_explosioneffect))
                {
            ...
            else if (hitvsp < 0)
            {
                ...
                with (gml_Script_instance_create(x, (y + 20), obj_explosioneffect))
                {
                    sprite_index = spr_fakepepsplasheffect
                    image_speed = 0.35
                    image_yscale = -1
    ...
    if (sprite_index == spr_fakepeppino_throwhead)
    {
        ...
        else
        {
            ...
            if (floor(image_index) >= 32 && (!instance_exists(headID)))
            {
                ...
                with (gml_Script_instance_create(x, (y - 20), obj_fakepephead))
                {
                    other.headID = id
                    thrown = 1
...
function scr_fakepepboss_backbreaker() //gml_Script_scr_fakepepboss_backbreaker
{
    ...
    if (vsp > 0 && sprite_index == spr_fakepeppino_fall && (!taunted))
    {
        ...
        with (gml_Script_instance_create(x, y, obj_fakepeptauntspawner))
        {
