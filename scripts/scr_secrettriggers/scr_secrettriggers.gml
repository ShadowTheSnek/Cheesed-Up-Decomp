...
function secret_open_portal(argument0) //gml_Script_secret_open_portal
{
    ...
    {
        if (trigger == argument0 && (!in_saveroom()) && (!(place_meeting(x, y, obj_marbleblock))) && (!(place_meeting(x, y, obj_secretblock))) && (!(place_meeting(x, y, obj_secretbigblock))) && (!(place_meeting(x, y, obj_secretmetalblock))) && (!(place_meeting(x, y, obj_secretdestroyable))))
            active = 1
    }
}
...
function secret_open_portalID(argument0) //gml_Script_secret_open_portalID
{
    if (argument0 == undefined)
        argument0 = obj_secretportal
    with (argument0)
        active = 1
}
