function scr_pausedraw() //gml_Script_scr_pausedraw
{
    var ui_index = 0
    if is_holiday((1 << 0))
        ui_index = 1
    if (fade > 0)
    {
        ...
        if (!instance_exists(obj_achievement_pause))
        {
            ...
            for (var i = 0; i < len; i++)
            {
                var b = pause_menu[i]
                var c = c_gray
                var t = gml_Script_lang_get_value(b)
                if (selected == i)
                {
    ...
    if (fade > 0 && lvlsave != -4 && lvlsave != "tutorial" && lvlsave != "exit" && lvlsave != "secretworld" && room != boss_fakepep && room != boss_fakepephallway && room != boss_fakepepkey && room != boss_vigilante && room != boss_noise && room != boss_pepperman && room != boss_pizzaface && room != Endingroom && room != Johnresurrectionroom && room != Creditsroom)
    {
        if (!instance_exists(obj_achievement_pause))
        {
            ...
            if (lvlsave == "etb")
                count = 2
            draw_text(((obj_screensizer.actual_width - 132) - 60), ((obj_screensizer.actual_height - 124) - 8), concat(secretcount, "/", count))
            draw_set_alpha(1)
        }
    }
