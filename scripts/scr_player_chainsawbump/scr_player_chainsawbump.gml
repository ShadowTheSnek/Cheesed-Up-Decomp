function scr_player_chainsawbump() //gml_Script_scr_player_chainsawbump
{
    ...
    if (hsp != 0 && movespeed > 0 && (sprite_index == spr_chainsawrev || sprite_index == spr_chainsawidle) && grounded)
        movespeed--
    ...
    if (!instance_exists(obj_chainsawpuff))
        gml_Script_instance_create(x, y, obj_chainsawpuff)
    if (floor(image_index) == (image_number - 1) && (sprite_index == spr_chainsawhit || sprite_index == spr_chainsawdash))
    {
        if key_attack
            state = states.mach2
        else
