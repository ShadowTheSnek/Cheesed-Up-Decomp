function scr_playersounds() //gml_Script_scr_playersounds
{
    ...
    if (state != states.tube && sprite_index != spr_knightpepstart && state != states.bombgrab && state != states.chainsaw && state != (297 << 0) && state != (296 << 0) && state != states.door && state != states.victory && state != states.stunned && state != states.gameover && state != states.fireass)
    {
    ...
    if (state == states.mach2 || state == states.mach3 || state == states.climbwall || state == states.rocket || (character == "S" && visible))
    {
        ...
        else
        {
            ...
            else if ((state == states.mach2 && sprite_index == spr_mach) || state == states.climbwall || (sprite_index == spr_snick_mach2 && grounded))
                s = (global.machsnd ? 6 : 2)
            else if ((state == states.mach3 && sprite_index != spr_crazyrun) || (sprite_index == spr_snick_mach3 && grounded))
    ...
    if ((sprite_index == spr_tumblestart || sprite_index == spr_tumble) && (state == states.tumble || character != "S"))
    {
    ...
    if (sprite_index == spr_playerSP_mach2jump)
    {
        ...
        fmod_event_instance_set_paused(flippingsnd, 0)
        sound_instance_move(flippingsnd, x, y)
    }
    else
        fmod_event_instance_stop(flippingsnd, 1)
}
